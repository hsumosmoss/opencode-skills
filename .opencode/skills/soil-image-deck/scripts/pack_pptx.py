...
def find_latest(images_dir: Path, prefix: str) -> str | None:
    """依 prefix 找圖片。支援兩種命名：
    - {prefix}_*.png  (e.g. 01_cover_1234.png)
    - {prefix}.png    (e.g. 01_cover.png)
    多個候選時取排序最後一個（最新）。
    """
    cands = sorted(glob.glob(str(images_dir / f"{prefix}_*.png")))
    if not cands:
        # fallback: 嘗試精確匹配 {prefix}.png
...
def pack_baked(images_dir: Path, output: Path):
    ...
    # 支援任意命名的 PNG（page_*.png 或 01_*.png 或其他）
    pngs = sorted(glob.glob(str(images_dir / "*.png")))
    # 排除 cropped 子目錄產生的檔案
    pngs = [p for p in pngs if "/cropped/" not in p and "\\cropped\\" not in p]
    if not pngs:
        raise SystemExit(f"錯誤：{images_dir} 找不到任何 .png 檔案")
