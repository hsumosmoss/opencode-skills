...
> [!CRITICAL]
> **MANDATORY CONTENT CUSTOMIZATION**
> The template components (`Video.tsx` and `Thumbnail.tsx`) contain placeholder texts like "视频封面标题", "要点一", or default switch cases.
> **Before launching Studio**, you MUST read the `podcast.txt` script and edit the copied `Video.tsx` and `Thumbnail.tsx` to map the specific sections of the script to actual React components. You MUST replace all placeholder titles, descriptions, and icons with the real content of the current video!
> Failure to do this will result in the video rendering with default template strings.
...
> When using `IconCard` or `Icon`, you MUST ensure that the requested icons are explicitly imported in `src/components/iconMap.ts`. Dynamic string lookups (e.g., `import * as LucideIcons`) will fail during Webpack building due to tree-shaking, resulting in icons rendering as fallback text. **Always modify `iconMap.ts` to explicitly import and map any new icons you use.**
