...
--- Frontmatter fields above are primarily for OpenCode Code / OpenClaw.
...
# `dependencies` is informational only — OpenCode Code does not auto-load skills
...
1. **Step 9**: Launch Remotion Studio — user reviews in real-time, requests changes until satisfied
   - **CRITICAL**: Before launching Studio, you MUST read the script (`podcast.txt`) and customize `Video.tsx` and `Thumbnail.tsx` to replace all template placeholder text with the actual video content, titles, and appropriate components. Do not launch Studio or render with the default placeholder text!
...
At Step 1 start, use your agent's task tracker (OpenCode Code `TaskCreate` / Codex todo list / equivalent) to create one task per step. Mark `in_progress` on start, `completed` on finish. Files in `videos/{name}/` (e.g. `podcast.txt`, `timing.json`, `output.mp4`) act as the durable record of what completed — if a session is interrupted, inspect the directory to determine where to resume.
